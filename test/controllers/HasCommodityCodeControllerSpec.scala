...
class HasCommodityCodeControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val hasCommodityCodeRoute = routes.HasCommodityCodeController.onPageLoad(NormalMode).url
  ...
  "HasCommodityCode Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(HasCommodityCodePage, true).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(true), NormalMode, "No name of goods found")(
          request,
          messages(application)
        ).toString
      }

...
class DescribeTheLegalChallengesControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new DescribeTheLegalChallengesFormProvider()
  val form         = formProvider()

  lazy val describeTheLegalChallengesRoute =
    routes.DescribeTheLegalChallengesController.onPageLoad(NormalMode, draftId).url

  lazy val saveDraftRoute: String =
    routes.DescribeTheLegalChallengesController.onSubmit(NormalMode, draftId, saveDraft = true).url

  lazy val continueRoute: String =
    routes.DescribeTheLegalChallengesController.onSubmit(NormalMode, draftId, saveDraft = false).url

  "DescribeTheLegalChallenges Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, describeTheLegalChallengesRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[DescribeTheLegalChallengesView]

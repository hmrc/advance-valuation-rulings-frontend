...
class DescribeTheConditionsControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new DescribeTheConditionsFormProvider()
  val form         = formProvider()

  lazy val describeTheConditionsRoute =
    routes.DescribeTheConditionsController.onPageLoad(NormalMode, draftId).url

  lazy val saveDraftRoute: String =
    routes.DescribeTheConditionsController.onSubmit(NormalMode, draftId, saveDraft = true).url

  lazy val continueRoute: String =
    routes.DescribeTheConditionsController.onSubmit(NormalMode, draftId, saveDraft = false).url

  "DescribeTheConditions Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, describeTheConditionsRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[DescribeTheConditionsView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        userAnswersAsIndividualTrader
          .set(DescribeTheConditionsPage, "answer")
          .success
          .value

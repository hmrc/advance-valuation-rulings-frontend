...
import models.{NormalMode, AdaptMethod, UserAnswers}
...
class AdaptMethodControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val adaptMethodRoute = routes.AdaptMethodController.onPageLoad(NormalMode).url
  ...
  "AdaptMethod Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(AdaptMethodPage, AdaptMethod.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(AdaptMethod.values.head), NormalMode)(request, messages(application)).toString
      }
    }

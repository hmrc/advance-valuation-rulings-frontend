...
class YourApplicationHasBeenCancelledControllerSpec extends SpecBase {
  ...
  "YourApplicationHasBeenCancelled Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.YourApplicationHasBeenCancelledController.onPageLoad().url)

        val result = route(application, request).value

        val view = application.injector.instanceOf[YourApplicationHasBeenCancelledView]

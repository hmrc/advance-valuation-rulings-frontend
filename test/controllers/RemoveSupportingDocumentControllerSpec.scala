...
class RemoveSupportingDocumentControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val removeSupportingDocumentRoute =
    routes.RemoveSupportingDocumentController.onPageLoad(NormalMode).url

  "RemoveSupportingDocument Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        UserAnswers(userAnswersId).set(RemoveSupportingDocumentPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, removeSupportingDocumentRoute)

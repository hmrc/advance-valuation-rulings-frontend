...
class DescribeTheRestrictionsControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val describeTheRestrictionsRoute = routes.DescribeTheRestrictionsController.onPageLoad(NormalMode).url
  ...
  "DescribeTheRestrictions Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, describeTheRestrictionsRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[DescribeTheRestrictionsView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(DescribeTheRestrictionsPage, "answer").success.value

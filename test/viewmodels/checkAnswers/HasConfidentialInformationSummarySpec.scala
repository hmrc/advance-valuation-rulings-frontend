...
class HasConfidentialInformationSummarySpec extends SpecBase {
  ...
  ".row" - {
    ...
    Seq(
      ...
      s"must create row for HasConfidentialInformationSummary when answer is $answer" in {

        val userAnswers: UserAnswers = userAnswersAsIndividualTrader
          .set(HasConfidentialInformationPage, answer)
          .success
          .value

        HasConfidentialInformationSummary.row(userAnswers) mustBe Some(

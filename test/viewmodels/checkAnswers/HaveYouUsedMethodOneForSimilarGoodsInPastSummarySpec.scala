...
class HaveYouUsedMethodOneForSimilarGoodsInPastSummarySpec extends SpecBase {
  ...
  ".row" - {
    ...
    Seq(
      ...
      s"must create row for HaveYouUsedMethodOneForSimilarGoodsInPastSummary when answer is $answer" in {

        val userAnswers: UserAnswers = userAnswersAsIndividualTrader
          .set(HaveYouUsedMethodOneForSimilarGoodsInPastPage, answer)
          .success
          .value

        HaveYouUsedMethodOneForSimilarGoodsInPastSummary.row(userAnswers) mustBe Some(

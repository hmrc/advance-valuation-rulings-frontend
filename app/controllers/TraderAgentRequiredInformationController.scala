...
import views.html.TraderAgentRequiredInformationView

class TraderAgentRequiredInformationController @Inject() (
  ...
  def onSubmit(draftId: DraftId): Action[AnyContent] =
    (identify andThen getData(draftId) andThen requireData).async {
      implicit request =>
        for {
          updatedAnswers <-
            Future.fromTry(request.userAnswers.set(TraderAgentRequiredInformationPage, true))
          _              <- userAnswersService.set(updatedAnswers)
        } yield Redirect(
          navigator.nextPage(TraderAgentRequiredInformationPage, NormalMode, updatedAnswers)
        )
    }
}

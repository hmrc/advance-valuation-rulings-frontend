...
class WhyTransactionValueOfSimilarGoodsController @Inject() (
  ...
  navigator: Navigator,
  actions: Actions,
  formProvider: WhyTransactionValueOfSimilarGoodsFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: WhyTransactionValueOfSimilarGoodsView
  ...
  def onPageLoad(mode: Mode, draftId: DraftId): Action[AnyContent] =
    (actions.identifyDraft(draftId)) {
      implicit request =>
        val preparedForm = WhyTransactionValueOfSimilarGoodsPage.fill(form)

        Ok(view(preparedForm, mode, draftId))
    }

  def onSubmit(mode: Mode, draftId: DraftId): Action[AnyContent] =
    (actions.identifyWithHistory(draftId, WhyTransactionValueOfSimilarGoodsPage)).async {
      implicit request =>
        form
          .bindFromRequest()
          .fold(

...
class ExplainReasonComputedValueController @Inject()(
                                        ...
                                        formProvider: ExplainReasonComputedValueFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: ExplainReasonComputedValueView
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(ExplainReasonComputedValuePage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(ExplainReasonComputedValuePage, mode, updatedAnswers))
      )
  }
}

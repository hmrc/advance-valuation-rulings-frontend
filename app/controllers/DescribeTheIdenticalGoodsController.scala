...
class DescribeTheIdenticalGoodsController @Inject() (
  override val messagesApi: MessagesApi,
  userAnswersService: UserAnswersService,
  navigator: Navigator,
  actions: Actions,
  formProvider: DescribeTheIdenticalGoodsFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: DescribeTheIdenticalGoodsView
  ...
  def onPageLoad(mode: Mode, draftId: DraftId): Action[AnyContent] =
    (actions.identifyDraft(draftId)) {
      implicit request =>
        val preparedForm = DescribeTheIdenticalGoodsPage.fill(form)

        Ok(view(preparedForm, mode, draftId))
    }

  def onSubmit(mode: Mode, draftId: DraftId): Action[AnyContent] =
    (actions.identifyWithHistory(draftId, DescribeTheIdenticalGoodsPage)).async {
      implicit request =>
        form
          .bindFromRequest()
          .fold(

package viewmodels.checkAnswers

import controllers.routes
import models.{CheckMode, UserAnswers}
import pages.HaveTheGoodsBeenSubjectToLegalChallengesPage
import play.api.i18n.Messages
import uk.gov.hmrc.govukfrontend.views.viewmodels.summarylist.SummaryListRow
import viewmodels.govuk.summarylist._
import viewmodels.implicits._

object HaveTheGoodsBeenSubjectToLegalChallengesSummary  {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(HaveTheGoodsBeenSubjectToLegalChallengesPage).map {
      answer =>

        val value = if (answer) "site.yes" else "site.no"

        SummaryListRowViewModel(
          key     = "haveTheGoodsBeenSubjectToLegalChallenges.checkYourAnswersLabel",
          value   = ValueViewModel(value),
          actions = Seq(
            ActionItemViewModel("site.change", routes.HaveTheGoodsBeenSubjectToLegalChallengesController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("haveTheGoodsBeenSubjectToLegalChallenges.change.hidden"))
          )
        )
    }
}

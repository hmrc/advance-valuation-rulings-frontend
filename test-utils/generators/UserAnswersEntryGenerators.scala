  ...
  implicit lazy val arbitraryAgentCompanyDetailsUserAnswersEntry: Arbitrary[(AgentCompanyDetailsPage.type, JsValue)] =
    Arbitrary {
      for {
        ...
        value <- arbitrary[AgentCompanyDetails].map(Json.toJson(_))
      } yield (page, value)
    }

  implicit lazy val arbitraryBusinessContactDetailsUserAnswersEntry
  ...
  implicit lazy val arbitraryWhatIsYourRoleAsImporterUserAnswersEntry
    ...
    Arbitrary {
      for {
        ...
        value <- arbitrary[WhatIsYourRoleAsImporter].map(Json.toJson(_))
      } yield (page, value)
    }
